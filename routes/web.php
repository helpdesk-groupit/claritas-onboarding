...
Route::middleware(['auth', \App\Http\Middleware\EnforceSingleSession::class, \App\Http\Middleware\SecurityAuditMiddleware::class, \App\Http\Middleware\EnforceTwoFactor::class])->group(function () {
    ...
    // Help / user-manual pages — shareable links, login required
    ...
    Route::post('/hr/claims/{claim}/reverse', [ExpenseClaimController::class, 'hrReverse'])->name('hr.claims.reverse');
    ...
    // Claim Reports page retired — "View report" now lives on My Claims. Redirect old links.
    Route::get('/my/claim-reports', fn () => redirect()->route('user.claims.index'))->name('user.claims.reports');
    ...
    Route::post('/my/claims/place-suggest', [ExpenseClaimController::class, 'placeSuggest'])->name('user.claims.place-suggest')->middleware('throttle:60,1');
    ...
    // Inline claim builder (My Claims page): each claim is an expandable card —
    // save its Category B details, add/remove items, and submit, all in place.
    Route::post('/my/claims/{claim}/inline-details', [ExpenseClaimController::class, 'inlineSaveDetails'])->name('user.claims.inline-details');
    // 60/min (not the 10/min upload limit): a multi-receipt statement is added one row at a
    // time, each re-uploading the shared image, so the batch needs headroom. Owner-only,
    // draft-only, malware-scanned — abuse risk is low.
    Route::post('/my/claims/{claim}/inline-item', [ExpenseClaimController::class, 'inlineAddItem'])->name('user.claims.inline-add-item')->middleware('throttle:60,1');
    Route::post('/my/claims/item/{item}/inline-update', [ExpenseClaimController::class, 'inlineUpdateItem'])->name('user.claims.inline-update-item')->middleware('throttle:60,1');
    ...
    // Dedicated review page (report + approve/reject + per-item flag & comment) — Manager & HR
    Route::get('/claims/{claim}/review', [ExpenseClaimController::class, 'reviewClaim'])->name('claims.review');
    ...
    Route::delete('/accounting/ai/scan/{scan}', [AiAccountingController::class, 'destroyScan'])->name('accounting.ai.destroy-scan');
    ...
    // Department re-route (manager of current dept / superadmin) — handled by the
    // re-route modal on the ticket detail page; only Department + Reason are editable.
    ...
    // Token usage + spend report (year › month › feature) and its per-month PDF export.
