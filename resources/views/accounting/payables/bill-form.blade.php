...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('accounting.bills.store') }}">
            @csrf
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-3"><label class="form-label">Vendor *</label>
                    <select name="vendor_id" class="form-select" required><option value="">Select</option>
                        @foreach($vendors ?? [] as $v)<option value="{{ $v->id }}">{{ $v->vendor_code }} - {{ $v->name }}</option>@endforeach
                    </select></div>
                ...
                <div class="col-md-2"><label class="form-label">Vendor Bill #</label><input type="text" name="vendor_bill_number" class="form-control" value="{{ old('vendor_bill_number') }}"></div>
            </div>
            ...
            <table class="table table-sm table-bordered" id="itemsTable" style="font-size:13px;">
                ...
                <tbody>
                    <tr class="item-row">
                        ...
                        <td><select name="items[0][account_id]" class="form-select form-select-sm"><option value="">—</option>
                            @foreach($accounts ?? [] as $a)<option value="{{ $a->id }}">{{ $a->account_code }} - {{ $a->name }}</option>@endforeach
                        </select></td>
                        <td><select name="items[0][tax_code_id]" class="form-select form-select-sm tax-select"><option value="" data-rate="0">No Tax</option>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i>Create Bill</button>
                <a href="{{ route('accounting.bills.index') }}" class="btn btn-outline-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('addItem').addEventListener('click', function() {
        ...
        if (!tr.querySelector('td:last-child button')) { tr.querySelector('td:last-child').innerHTML = '<button type="button" class="btn btn-outline-danger btn-sm py-0 px-1 remove-item"><i class="bi bi-x"></i></button>'; }
    ...
    function recalc() {
        ...
        tbody.querySelectorAll('.item-row').forEach(row => {
            ...
            const line = q * p; const t = line * r / 100;
