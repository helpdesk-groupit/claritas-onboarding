...
<div class="container-fluid py-4">
    ...
    @php $byMonth = $claims->groupBy(fn ($c) => $c->year.'-'.str_pad($c->month, 2, '0', STR_PAD_LEFT)); @endphp
    @forelse($byMonth as $monthKey => $monthClaims)
    @php
        [$gy, $gm] = explode('-', $monthKey);
        $monthLabel = \Carbon\Carbon::createFromDate($gy, $gm, 1)->format('F Y');
    @endphp
    <div class="month-group mb-3">
        <button type="button" class="month-head" data-bs-toggle="collapse" data-bs-target="#rpt-m-{{ $monthKey }}" aria-expanded="{{ $loop->first ? 'true' : 'false' }}">
        ...
        <div class="collapse {{ $loop->first ? 'show' : '' }}" id="rpt-m-{{ $monthKey }}">
            <div class="p-2 d-flex flex-column gap-2">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2" role="button" data-bs-toggle="collapse" data-bs-target="#rpt-{{ $claim->id }}">
                        <div>
                            <span class="fw-semibold"><i class="bi bi-bookmark-star me-1 text-primary"></i>{{ $claim->eventName() ?? 'General claim' }}</span>
                            <span class="text-muted small ms-2">{{ $claim->claim_number }}</span>
...
@include('partials.claim-month-style')
@endsection
