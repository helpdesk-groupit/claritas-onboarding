...
<div class="card">
    ...
    <div class="card-body border-bottom pb-3">
        <form action="{{ route('hr.offboarding.index') }}" method="GET" class="row g-2 align-items-end">
            <div class="col-md-2">
            ...
            <div class="col-auto d-flex gap-1">
                ...
                @if(request()->hasAny(['search','company','department']))
                    <a href="{{ route('hr.offboarding.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x"></i></a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover align-middle mb-0" style="font-size:13px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if(Auth::user()->isItManager() || Auth::user()->isSuperadmin())
                        @php $pastExitPic = $ob->exit_date && \Carbon\Carbon::today()->gt($ob->exit_date); @endphp
                        <td>
                            @if($pastExitPic)
                            ...
                            @else
                                <div class="d-flex align-items-center gap-1">
                                    <button type="button"
                                            ...
                                            data-bs-target="#picModalHROB{{ $ob->id }}">
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('hr.offboarding.show', $ob) }}" class="btn btn-sm btn-outline-primary" title="View">
                                    <i class="bi bi-eye"></i>
                                </a>
                                @if(Auth::user()->isHrManager() || Auth::user()->isSuperadmin())
                                @php $pastExit = $ob->exit_date && \Carbon\Carbon::today()->gt($ob->exit_date); @endphp
                                @if($pastExit)
                                    <button class="btn btn-sm btn-outline-secondary" disabled title="Exit date has passed">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                @else
                                    <a href="{{ route('hr.offboarding.edit', $ob) }}" class="btn btn-sm btn-outline-warning" title="Edit">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                @endif
                                @endif
...
{{-- ── Assign PIC Modals (HR Offboarding) ──────────────────────────────── --}}
@if(Auth::user()->isItManager() || Auth::user()->isSuperadmin())
@foreach($offboardings as $ob)
@php $pastExitPic = $ob->exit_date && \Carbon\Carbon::today()->gt($ob->exit_date); @endphp
@if(!$pastExitPic)
