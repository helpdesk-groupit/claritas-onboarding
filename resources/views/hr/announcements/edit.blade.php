        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    @if($editBody)
                    <textarea name="body" id="editBodyField" rows="5" maxlength="{{ $bodyMax }}"
                              class="form-control @error('body') is-invalid @enderror">{{ old('body', $announcement->body) }}</textarea>
                    @error('body')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    <div class="d-flex justify-content-end mt-1">
                        <span id="editBodyCounter" class="form-text"
                              data-max="{{ $bodyMax }}">{{ mb_strlen(old('body', $announcement->body ?? '')) }}/{{ $bodyMax }}</span>
                    </div>
                    @else
