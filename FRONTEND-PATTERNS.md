...
- `vendors/form.blade.php` — nonce'd IIFEs, `addEventListener` only. The e-waste type checkbox reveals nothing any more: the `.js-ewaste-only` "primary e-waste vendor" toggle it showed was retired on 2026-08-15 (the sweep RFQs every e-waste vendor), and its listener went with it. What remains is the SST exclusivity block — ticking "Not SST-registered" clears and disables the group boxes.
...
- Delete/re-read use the shared `form.js-confirm` + `data-confirm*` convention. These forms are rendered server-side and present at load, which matters: the confirm binder is `querySelectorAll` at load time, **not** delegation.
- **`_scan-js.blade.php` — upload → read → review → Save (added 2026-08-13).** One `@once`'d nonce'd IIFE binding every `[data-vnd-doc-form]` (both Add modals and every Edit modal, contracts and billing). Choosing a file POSTs it to `vendors.documents.scan` with a browser-generated token, then fills `[data-vnd-summary]`, `[data-vnd-companies]` and `[data-vnd-points]` from the JSON reply and enables `[data-vnd-save]`.
  - **The document form carries NO `enctype`**, because the file never travels with it — only the `scan_token` naming an upload the scan endpoint already stored. That is what lets the summary be reviewed before the record exists.
  - **A lost response is polled, not retried.** The read runs inside the upload request and can outlive the edge timeout on live, so a network-level failure falls back to `GET vendors.documents.scan-status?token=` (30 × 4s) rather than re-uploading and paying for the read twice. A failure the server *explained* (422/429) clears the file input instead — nothing was stored under that token.
  - **Every server-supplied value is written with `textContent`** (key points are `createElement`'d one `<li>` at a time). The summary is text from a third-party document; `innerHTML` here is the injection the module's own doctrine spends a paragraph guarding the model against.
  - **`hidden.bs.modal` resets the Add modal**, so a walked-away-from upload cannot be saved against a token the nightly sweep has since discarded. The Edit modals are deliberately not reset — their fields are the record.
  - Save on an Add form ships `disabled` and is enabled only by a settled scan **or** by a replayed `old('scan_token')` after a validation bounce; a scan that FAILED still enables it, because the file is stored and an unreadable document must stay fileable.
- **NO modal carries field inputs — not Add, not Edit.** The whole surface is: replace document · Companies Involved · Summary · Re-read, plus a lifecycle **Status** select on billing Edit only (our own workflow state, not something the document says). The record values come off the stored scan server-side. Don't add hidden ones "for completeness": both controllers ignore them, and a value the form cannot show is a value nobody checked.
- **Re-read sits in the Edit modal but its `<form>` is rendered outside it**, submitted through the button's `form="vndResummariseC{id}"` attribute — a `<form>` nested inside the modal's own form is invalid markup the browser drops, taking the fields with it. Same device as the AARF's two signatories.
