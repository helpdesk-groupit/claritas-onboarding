    ...
    protected static function detailPrompt(string $kind): string
    {
        $shape = match ($kind) {
            'contract' => '"title": string|null (a short name for this contract as the document itself titles it), '
                ...
                .'"start_date": "YYYY-MM-DD"|null (if the document lists several batches, units or '
                .'schedules each with their own start date, use the EARLIEST of them — the contract '
                .'as a whole is in force from when its first obligation begins), '
                .'"end_date": "YYYY-MM-DD"|null (if the document lists several batches, units or '
                .'schedules each with their own end date, use the LATEST of them, not the first — '
                .'the contract stays in force until its last obligation ends, and reporting an '
                .'earlier batch\'s end date would mark the whole contract expired while a later '
