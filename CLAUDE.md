...
**Coverage: a sweep reads the newest `message_limit` messages from the last `since_days` — it does NOT read history.** Knobs live in `config/email-workflow.php`. `since_days` (`EWF_SINCE_DAYS` = 30) bounds it absolutely: **mail older than the window is never captured, ever**. `message_limit` (`EWF_MESSAGE_LIMIT` = 500, 0 = unlimited) caps ONE sweep, and the cap is a **rolling-window design, not sampling**: each sweep re-reads the newest 500, the captures table's dedupe skips what it already has (re-reading costs nothing but time), and the window slides forward with the mailbox. While a day's volume stays well under the cap, every arriving message lands in at least one sweep before sliding past it — so **nothing new is missed**; the cap only leaves the pre-existing backlog unread, which is deliberate. At the observed ~35 msgs/day, 500 is ~2 weeks of catch-up slack.

**Don't warn merely for filling the cap — warn when the cap costs NEW mail.** `CaptureService::coverageWarning()` compares how far back a sweep reached against the *previous* sweep's start: reaching past it means the two runs' coverage overlaps and nothing was skipped (the quiet, healthy case, which is most runs — warning here would train the operator to ignore the warning). Only when a sweep no longer reaches the previous run has volume outgrown the cap, and the mail in between is read by no run, ever. That is persisted as `email_workflow_runs.coverage_warning` (the sweep is the only thing that knows the date of the oldest message it saw), flashed by Run-now, and surfaced in the run-history JSON — a **scheduled** sweep has no flash, so the history is the only place its warning can be read.

**Unlimited (`message_limit = 0`) is a trap on a large mailbox until `search()` streams.** Measured on the real Zoho mailbox: an unbounded sweep ran 25+ minutes and accumulated 180 MB+ before dying. Paging keeps the *raw* fetch flat (~40 MB), but the engine still holds every normalized message for the whole run, so memory and time grow with the mailbox. Making unlimited safe needs `EmailSourceAdapter::search()` to yield per message rather than return an array.

**`$paging['limit']` is a SWEEP total, never a page size — and 0 means unlimited.** Every provider caps a single page (Gmail `maxResults` ≤ 500, Graph `$top` ≤ 1000, IMAP fetch batch), so an adapter that forwards the limit as a page size and ignores the continuation token silently truncates. Gmail (`nextPageToken`) and Outlook (`@odata.nextLink`) both did exactly that until 2026-07-17; IMAP had the same class of bug via `fetch_order`. Newest-first ordering is likewise part of the `EmailSourceAdapter` contract, not an implementation detail. Unlimited is memory-safe because each adapter releases every page before fetching the next — peak tracks one page, not the sweep — but it costs TIME (~2 min per 100 IMAP messages): the synchronous **Run now** browser request will give up on a large mailbox even though the run completes server-side and lands in the run history. The scheduled sweep has no such limit (sync queue + CLI `max_execution_time` = 0); `RunEmailWorkflowCapture::$timeout` only binds if a real queue worker is ever enabled, which is why it is 3600 rather than the old 900.
