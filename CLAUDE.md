...
**Coverage: a sweep reads EVERY message in the last `since_days`, newest first — but it does NOT read history.** Knobs live in `config/email-workflow.php`. `since_days` (`EWF_SINCE_DAYS` = 30) is the real coverage decision: **mail older than the window is never captured, ever**. `message_limit` (`EWF_MESSAGE_LIMIT`) is **0 = unlimited** by default; a positive value is a deliberate cap, and hitting it skips the oldest mail *inside* the window permanently (the window slides forward). A capped sweep that reaches its ceiling logs a warning and says so in the run summary — never let that degrade into silent truncation, because "success, 0 captured" is exactly how every bug in this module has disguised itself.
