...
- **Why it changed:** `message_limit` used to be the coverage bound too, which is sound only while a day's volume stays under it. `admin@claritas.asia` (WF#4, Outlook/Graph, one sweep nightly at 19:30) exceeded it, so a sweep reached back only ~8–16 h and the mail between two runs was read by **neither**, then aged out of the window. Confirmed lost on 24, 27, 29, 30 Jul and 3, 4, 7 Aug 2026. The old `coverage_warning` detected it correctly every single time and nothing acted on it — **a warning nobody acts on is not a safeguard**, which is the whole reason the engine now acts.
